...
const Empresas: React.FC = () => {
  ...
  const handleSave = async (empresaData: Omit<Empresa, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Falha ao ${selectedEmpresa ? 'atualizar' : 'criar'} empresa.`);
      }
      handleCloseModals();
      fetchEmpresas(); // Refresh data
    } catch (err: any) {
      setError(err.message);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/${selectedEmpresa.id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
        throw new Error('Falha ao desativar empresa.');
      }
      handleCloseModals();
      fetchEmpresas(); // Refresh data
    } catch (err: any) {
  ...
  return (
    <div>
                  ...
                  {loading ? (
                    ...
                    <tr key={empresa.id}>
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <button onClick={() => handleOpenEditModal(empresa)} className="text-blue-600 hover:text-blue-900 mr-4"><PencilIcon className="h-5 w-5 inline-block"/></button>
                        <button onClick={() => handleOpenDeleteModal(empresa)} className="text-red-600 hover:text-red-900"><TrashIcon className="h-5 w-5 inline-block"/></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <ConfirmDeleteModal
        ...
        message={`Tem certeza de que deseja desativar a empresa "${selectedEmpresa?.nome}"? Esta ação não pode ser desfeita.`}
      />
    </div>
  );
};
